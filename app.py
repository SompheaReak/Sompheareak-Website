...
# This creates a file named 'shop.db' in your folder
...
class Product(db.Model):
    ...
    # We store lists as strings separated by commas (e.g., "LEGO,Toy")
...
def category(category_name):
    ...
    # Database Query: Find products where category string contains the name
...
def subcategory(subcategory_name):
    ...
    if request.args.get('ajax') == 'true':
        html = ""
        for p in products:
            # Note: We access p.image, p.name_kh (attributes) instead of p['image']
            html += f'''
            <div class="product-card">
                <img src="{p.image}" onclick="openImageModal(this.src)">
                <h3>{p.name_kh}</h3>
                <div class="price-line">{ "{:,}".format(p.price) }៛</div>
                <form class="add-to-cart-form">
                    <input type="hidden" name="product_id" value="{p.id}">
                    <input type="hidden" name="quantity" value="1">
                    <button type="submit" class="add-cart-button">🛒 Add</button>
                </form>
            </div>'''
        return html
...
def add_to_cart():
    try:
        ...
        # Convert DB object to dictionary for Session storage
        product_dict = {
            "id": p.id, "name_kh": p.name_kh, "price": p.price, "image": p.image
        }
        
        cart.append({"product": product_dict, "quantity": qty})
        session['cart'] = cart
...
# --- MODERN ADMIN ROUTES (Database Driven) ---
...
def add_product():
    ...
    if request.method == 'POST':
        ...
        new_p = Product(
            ...
            categories_str=request.form.get('categories', ''), # e.g. "LEGO,Toy"
...
if __name__ == '__main__':
    ...
    with app.app_context():
        db.create_all()
        # If DB is empty, fill with initial data
