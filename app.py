...
app = Flask(__name__)
app.secret_key = 'somphea_reak_studio_2025'
...
class Product(db.Model):
    ...
    # We store the main category here for grouping
...
@app.route('/')
def home():
    notify_telegram("🌐 *Visitor* on Custom Studio")
...
def admin_panel():
    ...
    grouped = {}
    for p in all_p:
        # Use a default if category is missing
...
def sync_catalog():
    ...
    for item in items:
        ...
        # Pick the first category from the list for grouping
        cat = item['categories'][0] if item.get('categories') else "General"
