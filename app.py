...
# --- AUTOMATIC DATA SYNC ---
# This function imports your hardcoded list from custom_bracelet into the database
def sync_hardcoded_data():
    if Product.query.first(): return # Only run if the database is empty
    
    print("🔄 Syncing hardcoded list into Database...")
    
    # PASTE YOUR FULL LIST FROM custom_bracelet.html HERE
    # I have added a few examples below based on your code
    hardcoded_charms = [
         // --- Charm
    ...
    for item in hardcoded_charms:
        new_p = Product(
            ...
            subcategory_str=item.get('subcategory', 'General'),
            stock=10 # Set initial stock for all items
        )
        db.session.add(new_p)
    
    db.session.commit()
    print("✅ Database is now synchronized with your product list!")
...
def admin_panel():
    ...
    # Group items for mobile category view
    grouped = {}
